...
const data = [
  {
    mode: 'markers',
    marker: { size: 10 },
    x: dataSource.map(d => d.value),
    y: ['Warming', 'Fire', 'Flood'],
    error_x: {
      symetric: false,
      array: dataSource.map(d => d.max - d.value),
      arrayminus: dataSource.map(d => d.value - d.min),
      width: 0
    },
    type: 'scatter',
  }
...
const layout = {
  ...
  yaxis: {
    ...
    tickmode: 'array',
    tickvals: [0, 1, 2],
    ticktext: dataSource.map(d => `<b>${d.title} (${d.number})</b>`
...
Plotly.newPlot('chart-1', data, layout, {  displayModeBar: false,
  responsive: true });